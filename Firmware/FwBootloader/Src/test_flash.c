...
// 0xFF (255)      fectory data present     0x01   // when this bit is set application can not program keys OR locations from 0xF8 to 0xFF
// 0xFE            Bl version               0x01
// 0xFD            reserved                 0xXX
// 0xFC (252)      device type              0xXX   // 0x01 : DEvmo device , 0x02 : wall switch
...
#define FDR_DAT_APP_VERSION_ADDRESS_LSB 0xF2   // eep write address are 1 byte
...
#define FECT_DAT_DEV_TYP_ADDRESS_LSB 0xFC
            ...
            { // process data 
                ...
                else if(frame[0] == WR_EEP) // write eeprom
                {
                    ...
                    if((frame[1u] > FDR_DAT_APP_VERSION_ADDRESS_LSB))  // user canot wite this once written without FDR
                    {
                          ...
                          else //its fectory data address 0xF8 to 0xFF if(frame[1u] == FECT_DAT_VALID_TYP_ADDRESS_LSB)
                          {
                              ...
                              Bt_ReadData(); // read eeprom
                              if((uint8)Bt_Data.ReadMem.result != 0xFF)
                              {
                                  flash_range_ok = 0; // rewriting fectory data not allowed
                              }
                              else
                              {
                                  //update BLE chip
                ...
                */
                else if((frame[0] == RD_EEP))
                {
                    ...
                    Bt_Data.ReadMem.typ = 1;
                    Bt_ReadData();            
                    frame[3] = Bt_Data.ReadMem.result;
                    frame[4] = 0;
                }
                ...
                */
                    ...
                    if(((uint8)Bt_Data.ReadMem.result == dev_type_ota) && ((uint8)Bt_Data.ReadMem.result != DEF)) //0xAA is appp valid else invalid // application valid flag is true, // NVM adress starts from 7000h to 70FFh last byte
                     {
                         ...
                         else
                         {
                             ...
                             Bt_Data.EepWr.add = FDR_DAT_APP_VALID_ADDRESS_LSB; 
                             Bt_Data.EepWr.eep_data = DEF;        
                             Bt_WriteEep();
                             // erase application version 
                             Bt_Data.EepWr.add = FDR_DAT_APP_VERSION_ADDRESS_LSB; 
                             Bt_Data.EepWr.eep_data = DEF;        
                             Bt_WriteEep();
                         }

                     }
                     tx_len = 0;  // cannot be removed
                }
                else
                {
                ...
                Bt_Data.Com.lent = tx_len; // all responces are of 5 byte long
