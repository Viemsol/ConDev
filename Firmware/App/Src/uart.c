...
void com_send_data(char *ptr, uint8 len)
{
    CRITICAL_EN;
    while(len)
    {
...
uint8 com_get_rx_buf_lnt(void) // gives the content length of rx buffer
{
	uint8 temp_dif;
	CRITICAL_EN;
    temp_dif = ((COM_TX_BUF-com.rx_tale) + com.rx_head);  // else part of below statemanr
	if(com.rx_head >= com.rx_tale)
    {
        temp_dif = (uint8_t)(com.rx_head - com.rx_tale);
    }
	CRITICAL_DIS;
	return(temp_dif);
}
...
// make sure legnth is rx known befor , function just reads rx buffer
void com_rx_read_data(uint8* dataPtr, uint8 len)
{
	while(len)
	{
        CRITICAL_EN;
		*dataPtr = com.rx_buf[com.rx_tale];
		INC_RX_TALE;
        CRITICAL_DIS;
		dataPtr++;
		len--;
...
{
    com.rx_buf[com.rx_head] = RX_BYTE;
